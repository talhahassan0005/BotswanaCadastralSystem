 ...
 *
 * Produces preview rows with a per-row status (valid | check | error) that
 * drives the Data Import screen's chips, matching the UI mockup.
 */
...
function parseNum(raw: string): number | null {
  ...
  if (s.includes(",") && s.includes(".")) {
    ...
    // Lone comma is the decimal separator.
    s = s.replace(",", ".");
  }
  const n = Number(s);
...
function splitLine(line: string): string[] {
  if (line.includes(",") && line.split(",").length >= line.split(/\s+/).length) {
    // Comma-delimited (most common). Keep simple — survey CSVs rarely quote.
...
export function parseSurveyCsv(text: string): ParseResult {
  ...
  // Detect header row by checking if the first row's cells match known aliases.
  ...
  if (hasHeader) {
    ...
    // No header: assume [beaconId, east, north, bearing, distance] by position.
  ...
  dataLines.forEach((line, i) => {
    ...
    // Validation rules (mirrors the Data Import + AI Validate screens).
    if (!row.beaconId) row.issues.push("missing beacon id");
    if (row.east == null || row.north == null) {
      // Coordinates may be computed later, but flag if also no bearing/distance.
