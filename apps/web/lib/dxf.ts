...
export interface ImportedDrawing {
  ...
  polylines: { pts: { x: number; y: number }[]; closed: boolean }[];
  texts: { x: number; y: number; text: string; height: number }[];
}
...
export function parseDxf(text: string): ImportedDrawing {
  ...
  let openPoly: { pts: { x: number; y: number }[]; closed: boolean } | null = null;
    ...
    switch (cur.type) {
      case "LINE": {
        ...
        if ([x1, y1, x2, y2].every((v) => v != null))
          out.polylines.push({ pts: [{ x: x1!, y: y1! }, { x: x2!, y: y2! }], closed: false });
        break;
      }
      case "LWPOLYLINE": {
        ...
        if (pts.length >= 2) out.polylines.push({ pts, closed: (flag & 1) === 1 });
        else if (pts.length === 1) out.points.push(pts[0]);
        break;
      }
      case "POLYLINE": {
        const flag = num(c, 70) ?? 0;
        openPoly = { pts: [], closed: (flag & 1) === 1 };
        break;
      }
      case "VERTEX": {
      ...
      case "POINT": {
        const x = num(c, 10), y = num(c, 20);
        if (x != null && y != null) out.points.push({ x, y });
        break;
      }
      case "TEXT":
      case "MTEXT": {
        ...
        if (x != null && y != null) out.texts.push({ x, y, text: cleanText(s), height: h ?? 10 });
        break;
      }
      case "CIRCLE": {
        ...
        if (x != null && y != null && r != null) {
          ...
          out.polylines.push({ pts, closed: true });
        }
        break;
      }
    }
  ...
  for (let k = start; k < end; k++) {
    ...
    if (code === 0) {
      ...
      if (t === "SEQEND") {
        ...
        const op = openPoly as { pts: { x: number; y: number }[]; closed: boolean } | null;
  ...
  flush();
  const tail = openPoly as { pts: { x: number; y: number }[]; closed: boolean } | null;
...
export function writeDxf(d: ImportedDrawing): string {
  ...
  for (const p of d.points) {
    e(0, "POINT"); e(8, "POINTS"); e(10, p.x); e(20, p.y); e(30, 0);
    if (p.label) {
      e(0, "TEXT"); e(8, "LABELS"); e(10, p.x); e(20, p.y); e(40, 1.5); e(1, p.label);
    }
  }
  for (const l of d.polylines) {
    e(0, "LWPOLYLINE"); e(8, "LINES"); e(90, l.pts.length); e(70, l.closed ? 1 : 0);
    for (const v of l.pts) {
      e(10, v.x); e(20, v.y);
    }
  }
  for (const t of d.texts) {
    e(0, "TEXT"); e(8, "TEXT"); e(10, t.x); e(20, t.y); e(40, t.height); e(1, t.text);
  }
  e(0, "ENDSEC");
  e(0, "EOF");
  return L.join("\r\n") + "\r\n";
