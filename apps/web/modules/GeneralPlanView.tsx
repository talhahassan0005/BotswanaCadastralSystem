...
export function GeneralPlanView() {
  ...
  const panelSectionLayouts = panelSections.map((section, si) => {
    ...
    const rowYs = section.rows.map((row) => {
      ...
      // disturb na karna."
      panelCursorY += row.heading ? 6 : 6; // client req 2026-09-05: "inke darmayan space ur kam karo heading ur text ke darmayan" (10 -> 6)
      return y;
    });
            ...
            <>
              {section.rows.map((row, i) => (
                <text
                  ...
                  fontSize={row.heading ? Math.min(panelHeadingFS, 6 * 0.65) : Math.min(panelFS, 12 * 0.65)}
        ...
        {sheetMode === "general" && (
          <>
            ...
            {groupUsedBeacons.length > 0 && (() => {
              ...
              // wali row dekho isko bi baqi rows ki atarah hona chahiye na
              // buss heading bold ho ur kuch nahi" — replacing several
              // rounds of hand-picked custom offsets, one of which had left
              ...
              // chars, from fmtCoord). Ratio/margin tightened further
              // (client req 2026-09-07, second round: "columns main space
              // ur reduce karo") — 0.52 -> 0.46 and the extra clearance
              // margin 1 -> 0 (bcPad alone is still the left-padding before
              // the text itself); digits in most sans-serifs render closer
              // to 0.45-0.5em, so this is close to the real minimum rather
              // than a safety-padded estimate.
              const BC_CHAR_W_RATIO = 0.46;
              const bcMinGroupWForCol0 = (9 * bcFS * BC_CHAR_W_RATIO + bcPad) / 0.22;
              const bcMinGroupWForCol12 = (13 * bcFS * BC_CHAR_W_RATIO + bcPad) / (0.61 - 0.22);
